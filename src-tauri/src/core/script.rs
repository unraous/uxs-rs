...
macro_rules! inject_script {
    ($script_path:expr, { $($key:ident : $value:expr),* $(,)? }) => {{
        let original_script = include_str!($script_path);
        
        let args = serde_json::json!({
            "scriptName": std::path::Path::new($script_path).file_name().unwrap().to_str(),
            // 假设你已经定义了 stringize! 宏，否则用 stringify!
            $(stringify!($key) : $value),* 
        });
        ...
        format!(
            ...
            (function(args) {{
                try {{
                    ...
                    console.error("[Tauri Script Error]", e);
                }}
            }})({});
            "#,
            original_script,
...
pub fn obtain(t: Type) -> Option<String> {
    match t {
        Type::Course => Some(
            inject_script!("../scripts/core.js", {
                launchOption: CONFIG.options.launch_option,
                forceSpeed: CONFIG.options.force_speed,
